...
# png(file="distsatotal202206.ggplot.annotated.png")
ggplot( df2, aes(x=LONGITUDE, y=LATITUDE)) + geom_point( color=rgb(0.7,0.7,0.7),shape=1, size=2 ) +
  ggtitle("June 2022 604k ebird observations" ) + theme(plot.title = element_text(hjust = 0.5)) +
  annotate('text', x=c(-77, -73.3, -69.2, -72, -74, -75.5, -68.1, -46.6, -43.2, -60, -80, -70.7, -58.4, -68.3, -78.5, -72.9), y=c(-12, -3.7, -12.6, -13.5, 4.7, 10.4, -16.5, -23.6, -22.9, -3.1, -6.8, -33.4, -34.6, -54.8, 0.2, -41.5), label=c('L', 'I', 'PM', 'C', 'B', 'C', 'LP', 'SP', 'RdJ', 'M', 'C', 'S', 'BA', 'U', 'Q', 'PM' ))
# dev.off()
# system( 'open -a Preview.app distsatotal202206.ggplot.annotated.png')

# Output a subset of the variables ------------------------------------------------------------

df2_subset <- df2[ c(1,6:7,11,17,29:31) ]
# write_csv( df2_subset, file="ebirdsouthamerica202206.csv", row.names = TRUE )
